...
impl NodeEngine {
    ...
    /// Delete all nodes whose metadata.software field matches `software` (e.g. "houdini").
    /// Used by the VFX bridge to replace an existing software import.
    pub fn delete_by_software(&self, software: &str) -> CortexResult<usize> {
        let conn = self.pool.get().map_err(|e| CortexError::Database(e.to_string()))?;
        let deleted = conn.execute(
            "DELETE FROM nodes WHERE json_extract(metadata, '$.software') = ?1",
            [software],
        )?;
        Ok(deleted)
    }
    ...
    pub fn generate_seed_sql_to(&self, out: &std::path::Path) -> CortexResult<String> {
        ...
        let mut sql = String::from(
            "-- CORTEX node seed — auto-generated, do not edit manually.\n\
             -- Regenerate via Admin → Export as Seed after importing from your DCCs.\n\n"
        );
            ...
            let row = format!(
                "INSERT OR IGNORE INTO nodes \
                 (id,vault_id,software_id,name,display_name,category,object_type,\
                  description,version,color,icon,tags,inputs,outputs,parameters,\
                  documentation,notes,production_tips,media_ids,is_deprecated,metadata,\
                  created_at,updated_at) VALUES ({},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},0,{},{},{});\n",
                sql_str(&node.id.to_string()),
                sql_opt_str(&node.vault_id.map(|u| u.to_string())),
                sql_opt_str(&node.software_id.map(|u| u.to_string())),
                sql_str(&node.name),
                sql_str(&node.display_name),
                sql_str(&serde_json::to_string(&node.category).unwrap_or_default()),
                sql_str(&serde_json::to_string(&node.object_type).unwrap_or_default()),
                sql_opt_str(&node.description),
                sql_opt_str(&node.version),
                sql_opt_str(&node.color),
                sql_opt_str(&node.icon),
                sql_str(&serde_json::to_string(&node.tags).unwrap_or_default()),
                sql_str(&serde_json::to_string(&node.inputs).unwrap_or_default()),
                sql_str(&serde_json::to_string(&node.outputs).unwrap_or_default()),
                sql_str(&serde_json::to_string(&node.parameters).unwrap_or_default()),
                sql_opt_str(&node.documentation),
                sql_opt_str(&node.notes),
                sql_str(&serde_json::to_string(&node.production_tips).unwrap_or_default()),
                "'[]'",
                sql_str(&node.metadata.to_string()),
                sql_str(&node.created_at.to_rfc3339()),
                sql_str(&node.updated_at.to_rfc3339()),
            );
            sql.push_str(&row);
        }
...
/// Escape a string for SQL: wraps in single quotes, doubles any internal quotes.
fn sql_str(s: &str) -> String {
    format!("'{}'", s.replace('\'', "''"))
}

/// Same as sql_str but emits NULL for None.
